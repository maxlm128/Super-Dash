...
// looks for an integer in the array trailmap and sets every value thats over 0 one lower and if the integer reaches 0, the trail is deleted by executing the method printTrail()
...
//checks for every case if something is in front of the player
	public boolean isSomethingInFront(char pRotation, char pContent) {
		switch (pRotation) {
		case 'd':
